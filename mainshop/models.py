...
def get_product_url(obj, view_name):
    ...
    print(ct_model)
    return reverse(view_name, kwargs={"ct_models": ct_model, "slug": obj.slug})
...
class CategoryManager(models.Manager):
    ...
    def get_categories_for_left_sidebar(self):
        type_models = get_models_for_count("notebook", "smartphone")
        qs = list(self.get_queryset().annotate(*type_models))
        data = [dict(name=c.name, url=c.get_absolute_url(), count=getattr(c, self.CATEGORY_NAME_COUNT[c.name])) for c in
                qs]
        return data
